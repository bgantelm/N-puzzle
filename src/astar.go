...
func astar(tab [][]int, goodTab [][]int, len int, choice int) int  {
  ...
  if choice == 1 { openList[0].h = calculeManhattan(openList[0], goodTab)  * acknowledge
  } else if choice == 2 { openList[0].h = misplaced(openList[0], goodTab)  * acknowledge } else { openList[0].h = h_row_column(openList[0], goodTab)  * acknowledge }
  var nbrState = 1
  var nbrMove = 0
  var indexState = 1
  var index = 1
        ...
        endfunc(closeList, best, len, nbrState, nbrMove, indexState);   return 1
          ...
          for l < cr {
            ...
            if err == 0 && err1 == 0  {
              ...
              if choice == 1 { newList.h = calculeManhattan(newList, goodTab) * acknowledge
              } else if choice == 2 { newList.h = misplaced(newList, goodTab) * acknowledge
              } else { newList.h = h_row_column(newList, goodTab) * acknowledge }
